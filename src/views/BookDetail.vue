<script setup lang="ts">
import { onMounted } from "vue";
import TopHeader from "@/components/TopHeader.vue";
import LoggedLayout from "@/components/LoggedLayout.vue";
import { router } from "@/router/router";
import { useBookStore } from "@/store/book.store";
import { storeToRefs } from "pinia";
import { FwbSpinner } from "flowbite-vue";

const bookStore = useBookStore();

const { getBookDetail, isLoadingBookDetail } = storeToRefs(bookStore);

onMounted(() => {
  bookStore.getBookDetails(router.currentRoute.value.params.id as string);
});
</script>

<template>
  <LoggedLayout>
    <TopHeader />
    <div class="flex justify-center">
      <FwbSpinner v-if="isLoadingBookDetail" size="12" />
      <div v-else class="flex flex-col w-full p-4 gap-8">
        <div class="flex items-center gap-4">
          <img
            :src="getBookDetail.book_cover"
            :alt="getBookDetail.id"
            class="w-48 rounded-md"
          />
          <div class="flex flex-col gap-4 w-full">
            <div class="flex items-center justify-between gap-4">
              <h1>{{ getBookDetail.title }}</h1>
              <span class="bg-orange-coral px-4 py-2 rounded-lg font-bold">
                {{ getBookDetail.book_type }}
              </span>
            </div>
            <h3>{{ getBookDetail.author_name }}</h3>
            <p>ISBN: {{ getBookDetail.isbn }}</p>
            <div class="flex gap-4 border-t border-t-black pt-4 items-center">
              <button
                class="bg-yellow-mustard hover:bg-orange-coral transition ease-in-out text-base font-bold rounded-lg px-4 py-2 flex items-center justify-center gap-2"
              >
                <img
                  src="/icons/heart_solid.svg"
                  alt="Wishlist heart"
                  class="w-4"
                />
                Add to wishlist
              </button>
              <button
                class="bg-yellow-mustard hover:bg-orange-coral transition ease-in-out text-base font-bold rounded-lg px-4 py-2 flex items-center justify-center gap-2"
              >
                <img
                  src="/icons/cart_plus_solid.svg"
                  alt="Wishlist heart"
                  class="w-5"
                />
                <p v-if="getBookDetail.price">
                  Add to cart for
                  {{
                    getBookDetail.price.toLocaleString("en-US", {
                      style: "currency",
                      currency: "IDR",
                      currencyDisplay: "narrowSymbol"
                    })
                  }}
                </p>
                <p v-else>Add to cart</p>
              </button>
              <p class="font-bold">
                Stock: {{ getBookDetail.current_stock }} /
                {{ getBookDetail.original_stock }}
              </p>
            </div>
            <div class="flex gap-4">
              <div class="flex items-center gap-2">
                <p class="font-bold">Category:</p>
                <span class="bg-orange-coral text-sm px-2 rounded-md">
                  {{ getBookDetail.category_name }}
                </span>
              </div>
              <div class="flex items-center gap-2">
                <p class="font-bold">Genre:</p>
                <span class="bg-orange-coral text-sm px-2 rounded-md">
                  {{ getBookDetail.genre_name }}
                </span>
              </div>
              <div class="flex items-center gap-2">
                <p class="font-bold">Language:</p>
                <span class="bg-orange-coral text-sm px-2 rounded-md">
                  {{ getBookDetail.language_name }}
                </span>
              </div>
              <p class="font-bold">
                Publication:
                {{ new Date(getBookDetail.publication_year).getFullYear() }}
              </p>
            </div>
          </div>
        </div>
        <div class="flex flex-col w-full gap-4">
          <h3>Description</h3>
          <p>{{ getBookDetail.description }}</p>
        </div>
      </div>
    </div>
  </LoggedLayout>
</template>
